...
class DirichletBC():
    ...
    def __init__(self, space, val, subdomain, method='topological'):
        ...
        for i in range(0, self.si):
            localrowoffset = localrowoffset + space._parent.get_space(i).get_localghostedndofs()

        for direc in direclist:
            rows = space.get_space(self.si).get_boundary_indices(direc)  # returns split local rows

            split_global_rows = space._parent.get_space(self.si).get_lgmap.apply(rows) # turns rows from split local into split field global
            global_rows = space._parent.get_field_lgmap(self.si).apply(rows) # turn rows from split local into monolithic global
            
            self._splitfieldglobalrows.append(split_global_rows)  
            self._globalrows.append(global_rows)  
