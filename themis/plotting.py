...
def get_plotting_spaces(mesh, nquadplot, nquadplot_v=None):
    ...
    if mesh.ndim == 1:
        scalarelem = FiniteElement("DG", interval, nquadplot-1, variant='feec')
    if mesh.ndim == 2 and not mesh.extruded:
        scalarelem = FiniteElement("DQ", quadrilateral, nquadplot-1, variant='feec')
    if mesh.ndim == 2 and mesh.extruded:
        scalarelem = TensorProductElement(FiniteElement("DG", interval, nquadplot-1, variant='feec'), FiniteElement("DG", interval, nquadplot_v-1, variant='feec'))
    if mesh.ndim == 3 and not mesh.extruded:
        scalarelem = FiniteElement("DG", hexahedron, nquadplot-1, variant='feec')
    if mesh.ndim == 3 and mesh.extruded:
        scalarelem = TensorProductElement(FiniteElement("DG", quadrilateral, nquadplot-1, variant='feec'), FiniteElement("DG", interval, nquadplot_v-1, variant='feec'))
    ...
    middle = Pascal1D(1)
    if mesh.ndim == 1:
        opts = [sptsl2, middle, middle]
    if mesh.ndim == 2 and not mesh.extruded:
        opts = [sptsl2, sptsl2, middle]
    if mesh.ndim == 2 and mesh.extruded:
        opts = [sptsl2, sptsl2v, middle]
    if mesh.ndim == 3 and not mesh.extruded:
        opts = [sptsl2, sptsl2, sptsl2]
    if mesh.ndim == 3 and mesh.extruded:
        opts = [sptsl2, sptsl2, sptsl2v]
