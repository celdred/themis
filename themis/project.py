...
class Projector():
    ...
    def __init__(self, v, v_out, bcs=None, options_prefix=None, solver_parameters=None):
		...
		if not self._same_fspace:
			...
			#maxdegree = V.themis_element().maxdegree()
			#maxdegree = 2*maxdegree+1
			maxdegree = 3
			#FIX THIS
						
			lhs = ufl.inner(p, v_out)*ufl.dx(degree=maxdegree)
			
			if len(vlist) == 1:	
				rhs = ufl.inner(p, v)*ufl.dx(degree=maxdegree)
			else:
				plist = ufl_expr.TestFunctions(V)
				rhs = 0
				for i,p in enumerate(plist):
					if not (vlist[i] == 0):
						rhs = rhs + ufl.inner(p, vlist[i])*ufl.dx(degree=maxdegree)
