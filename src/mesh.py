...
class SingleBlockMesh(MeshBase):
	...
	def __init__(self,nxs,bcs,name='singleblockmesh',coordelem=None):
		...
		newcoords = create_uniform_nodal_coords(self.get_cell_da(0),nxs,pxs,lxs,dxs,bcs,localnxs)
...
def create_uniform_nodal_coords(dm,nxs,pxs,lxs,dxs,bcs,localnxs):
	...
	zmax = 0.0
	if len(pxs) >= 1:
		xmin = 0.5*dxs[0] + pxs[0]-lxs[0]/2.
		if bcs[0] == 'periodic': xmax = 0.5*dxs[0] + pxs[0]+lxs[0]/2.
		if bcs[0] == 'nonperiodic': xmax = -0.5*dxs[0] + pxs[0]+lxs[0]/2.
	if len(pxs) >= 2:
		ymin = 0.5*dxs[1] + pxs[1]-lxs[1]/2.
		if bcs[0] == 'periodic': ymax = 0.5*dxs[1] + pxs[1]+lxs[1]/2.
		if bcs[0] == 'nonperiodic': ymax = -0.5*dxs[1] + pxs[1]+lxs[1]/2.
	if len(pxs) >= 3:
		zmin = 0.5*dxs[2] + pxs[2]-lxs[2]/2.
		if bcs[0] == 'periodic': zmax = 0.5*dxs[2] + pxs[2]+lxs[2]/2.
		if bcs[0] == 'nonperiodic': zmax = -0.5*dxs[2] + pxs[2]+lxs[2]/2.
	...
	for i in range(len(pxs)):
		#if bcs[i] == 'periodic':
		coords1D = np.tile([-0.5*dxs[i],0.5*dxs[i]],localnxs[i]) #This is DG1 ie 2 dofs per element
